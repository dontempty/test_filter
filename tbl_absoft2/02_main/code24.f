...
c          DT=DTR/20.   ! FOR STABLE START WITH RANDOM NOISE FIELD
      ELSE  
          CALL CFL(Q1,Q2,Q3,CFLM)
...
c---- write ins-field for post-processing
c      goto 80  ! skip write ins
 
      if (mt.ne.0) then  ! for pb
        num_moni_ph=8
        iphase = mod(ntime_ctd,mt+1)
        if( mod(iphase,int((mt+1)/num_moni_ph)).eq.0
     &    ) then
            num_ins = num_ins + 1
           if (num_ins.le.2000) then
             CALL WRITEUP2(Q1,Q2,Q3,P)
             write(*,*) num_ins,iphase, q2(200,1,n3m/2)
           endif
        endif
      endif


      if (mt.eq.0) then  ! for sb, nb
        if( mod(ntime,5).eq.0
     &    ) then
            num_ins = num_ins + 1
           if (num_ins.le.2000) then
...
10    CONTINUE
      ...
      THETA=1.0    ! FOR MAIN SIMULATION
C      THETA=620./RE ! FOR INFLOW, WE SHOULD SPECIFY THE INLET RE    
...
C---  RANDOM FLUCTUATIONS
      ...
      DO I=0,N1
       ...
       Q1(I,J,K)=VPER*DRNUNF()
       Q2(I,J,K)=VPER*DRNUNF()
       Q3(I,J,K)=VPER*DRNUNF()
...
c      OPEN(10,file=filename,STATUS='UNKNOWN',FORM='UNFORMATTED'
c     &       ,action='write')
c      WRITE(10) (((Q1(I,J,K),Q2(I,J,K),Q3(I,J,K)
c     >            ,K=1,N3M),J=0,N2),I=0,N1)
c      WRITE(10) (((P(I,J,K),K=1,N3M),J=1,N2M),I=1,N1M)
c      CLOSE(10)

c---  to reduce the file size, region neibhoring the slot is written.
      IX1 = 171  ! X=65
      IX2 = 276  ! X=105
      JY1 =   0  ! Y=0
      JY2 =  49  ! Y=9
     ...
     >            ,K=1,N3M),J=JY1,JY2),I=IX1,IX2)
      WRITE(10) (((P(I,J,K),K=1,N3M),J=JY1+1,JY2),I=IX1,IX2)
      CLOSE(10)
...
c      I_SAVE=65         ! LES RE=1410
     ...
     &       (ntime-1)/len_inflow
      ...
      WRITE(80)   
     >  ((Q1(I_SAVE  ,J,K),J=1,N2M),K=1,N3M)
     > ,((0.5*(Q2(I_SAVE-1,J,K)+Q2(I_SAVE,J,K)),J=2,N2M),K=1,N3M)
     > ,((0.5*(Q3(I_SAVE-1,J,K)+Q3(I_SAVE,J,K)),J=1,N2M),K=1,N3M)
...
c-PB
      PERIOD=real(mt+1)*DT
      IF (I.GE.IS.AND.I.LT.IE) 
     >    UBC1(I,K,2)=A0*(1.+sin(2.0*PI*TIME/PERIOD+1.5*PI))
c-SB
...
cC     FOR INFLOW GENERATION
c      UBC2(I,K,1)=Q1(I,N2M,K)  ! Q1(I,N2,K)
cC      UBC2(I,K,2)=Q2(I,N2M,K) ! Q2(I,N2,K)  -> SEE GET_INFLOW
c      UBC2(I,K,3)=Q3(I,N2M,K)  ! Q3(I,N2,K)
C
C     FOR MAIN SIMULATION
      UBC2(I,K,1)=1.0  ! Q1(I,N2,K)
      UBC2(I,K,2)=Q2(I,N2M,K)  ! Q2(I,N2,K)
...
c      CALL RESCALING(Q1,Q2,Q3,TIME)              ! INFLOW SIMULATION
c      GOTO 88
...
C     READ THE INFLOW DATA FROM THE SAVED FILES
      ...
      IF (MOD(NTIME-1,LEN_INFLOW).EQ.0) THEN
        ...
        filename=
     & '/home/kkim/pb_tbl/re670/inflow/02_save/INFLOW.'
 ...
 11   QH1(I,J,K)=R1(I,J)
  1   CONTINUE
     ...
     >       -0.5*DX3*W1*0.5)*DT
      ...
      R3(I,K)=QH1(I,J,K)
  30  CONTINUE
      CALL CTDMA3I(AMK,ACK,APK,R3,QH1,J,N3M,2,N1M)
  3   CONTINUE
 ...
 11   QH2(I,J,K)=R1(I,J)
  1   CONTINUE
     ...
     >       )*DT
      ...
      R3(I,K)=QH2(I,J,K)
  30  CONTINUE
      CALL CTDMA3I(AMK,ACK,APK,R3,QH2,J,N3M,1,N1M)
  3   CONTINUE
 ...
 21   QH3(I,J,K)=R2(I,J)
  2   CONTINUE
     ...
     >       )*DT
      ...
      R3(I,K)=QH3(I,J,K)
  30  CONTINUE
      CALL CTDMA3I(AMK,ACK,APK,R3,QH3,J,N3M,1,N1M) 
  3   CONTINUE
...
C*************** EDDY_VISCOS_DM **************************************
      ...
      REAL SRT11(M1,M3)   ! STRAIN RATE SR_IJ=1/2*(DU_I/DX_J+DU_J/DX_I)
      REAL SRT22(M1,M3)   ! STRAIN RATE SR_IJ=1/2*(DU_I/DX_J+DU_J/DX_I)
      REAL SRT33(M1,M3)   ! STRAIN RATE SR_IJ=1/2*(DU_I/DX_J+DU_J/DX_I)
      REAL SRT12(M1,M3)   ! STRAIN RATE SR_IJ=1/2*(DU_I/DX_J+DU_J/DX_I)
      REAL SRT13(M1,M3)   ! STRAIN RATE SR_IJ=1/2*(DU_I/DX_J+DU_J/DX_I)
      REAL SRT23(M1,M3)   ! STRAIN RATE SR_IJ=1/2*(DU_I/DX_J+DU_J/DX_I)
...
C---- INCLUDE POISSON SOLVING ROUTINE 

c      INCLUDE 'POISSON_MG_IMSL.F'
      INCLUDE 'POISSON_CS_IMSL.F'
